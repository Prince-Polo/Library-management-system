...
import APIs.SeatAPI.{ConfirmedSeatQueryMessage, ConfirmedSeatQueryResponse}
import Common.SeatInfo
import Common.SeatStatus
import Common.SeatStatus.SeatStatus // 导入 SeatStatus

case class ConfirmedSeatQueryPlanner(message: ConfirmedSeatQueryMessage, override val planContext: PlanContext) extends Planner[String] {
    ...
    readDBRows(
      ...
      val seats = rows.map { row =>
        SeatInfo(
          ...
          row.hcursor.get[SeatStatus]("status").getOrElse(SeatStatus.Confirmed),
