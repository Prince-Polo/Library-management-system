...
import Common.API.{PlanContext, TraceID, Message}
import Common.Object.{ParameterList, SqlParameter}
import cats.effect.*
import io.circe.{Decoder, Encoder, HCursor, Json}
import io.circe.generic.auto.*
...
package object DBAPI {
  ...
  def startTransaction[A](block: PlanContext ?=> IO[A])(using encoder: Encoder[A], ctx: PlanContext): IO[A] = {
    ...
    def commitOrRollbackAction(result: Either[Throwable, A]): IO[A] =
      result match {
        ...
        case Left(exception) =>
          EndTransactionMessage(false).send >> IO.raiseError(DidRollbackException(exception))
        case Right(value) =>
          if (ctx.transactionLevel == 0)
            EndTransactionMessage(true).send.as(value)
          else IO.pure(value)
    ...
    for {
      ...
      result <- block(using newContext).attempt
      finalResult <- commitOrRollbackAction(result)
    } yield finalResult
  }
  ...
  def writeDBReturningId(sqlQuery: String, parameters: List[SqlParameter])(using PlanContext, connection: Connection): IO[Int] = {
    WriteDBReturningIdMessage(sqlQuery, parameters).send(connection)
  }
