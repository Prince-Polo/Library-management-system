...
import { useSeatData } from 'Pages/LibraryDataLinking';
                    ...
                    <div style={seatStyle}>
                        {sortedSeats.map((seat) => (
                            <button
                                ...
                                onClick={() => handleSeatClick(seat.seatNumber, seat.occupied === 'true', seat.feedback,seat.status==='Confirmed')}
                            >
                                <span>{`F${index}${area}${seat.seatNumber.padStart(3, '0')}`}</span>
                                <span style={{ color: seat.occupied === 'true' ? 'red' : seat.status === 'Confirmed' ? '#a60924' : seat.status === 'Reported' ? '#ecc334' : '#4cac34', fontSize: '30px' }}>●</span>
                            </button>
                        ))}
                    </div>
                </div>
