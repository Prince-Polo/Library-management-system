...
// 管理员发布任务
case class AddJobPlanner(JobShortDescription: String, JobLongDescription: String, JobHardness: Int, JobCredit: Int, JobRequired: Int, override val planContext: PlanContext) extends Planner[String] {
  override def plan(using planContext: PlanContext): IO[String] = {
    writeDB(
      s"INSERT INTO $schemaName.jobs ( jobShortDescription, jobLongDescription, jobHardness, jobCredit, jobRequired, jobCurrent) VALUES (?, ?, ?, ?, ?, ?)",
      List(
      ...
      //  SqlParameter("String2DArray", "{}") // 初始二维数组为空
      )
    ).map { rowsAffected =>
