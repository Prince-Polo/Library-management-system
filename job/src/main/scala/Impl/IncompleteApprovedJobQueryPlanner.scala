...
import APIs.JobAPI.{JobInfo, IncompleteApprovedJobQueryResponse, IncompleteApprovedJobQueryMessage}

case class IncompleteApprovedJobQueryPlanner(message: IncompleteApprovedJobQueryMessage, override val planContext: PlanContext) extends Planner[String] {
  override def plan(using planContext: PlanContext): IO[String] = {
    readDBRows(
      s"""
         ...
         |WHERE jobComplete = 'FALSE' AND jobApproved = 'TRUE'
         |ORDER BY jobId
         """.stripMargin,
      List()
    ).map { rows =>
      ...
      IncompleteApprovedJobQueryResponse(jobs).asJson.noSpaces
    }
  }
}
